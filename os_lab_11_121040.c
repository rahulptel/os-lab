//This C Programs to implement Earliest Deadline First (EDF) scheduling algorithm of Real
...
#include<stdio.h>
int main(){
	int N,burst,i,j,time,exec,deadline,total,flag;
	...
	for(time=0;time<total;time++){		
		...
		for(i=0;i<N;i++){
			...
			if(process[i][2]>0){		
				...
				if(process[i][0]<=time){
					if(flag==0){	
						deadline = process[i][1];
						exec = i;
						flag = 1;
					}
					//If the process has still execution time left, than we check whose deadline is
					//more close from the current time 
					if(process[i][1]<deadline){
						exec = i;
					}
				}
				//If the process has successfully executed we increment the finished counter which 
				//which contains the number of process who have finished their execution		
				else{
					break;					
				}
			}
